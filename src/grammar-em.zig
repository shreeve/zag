...
const LexerGenerator = struct {
    ...
    fn generateNumberScanner(self: *LexerGenerator) !void {
        ...
        if (has_decimal or has_exponent or has_zdigits or has_leading_dot) {
            ...
            if (has_decimal or has_exponent or has_leading_dot) {
                try self.write("if (has_decimal or has_exponent");
                if (has_leading_dot) try self.write(" or starts_with_dot");
                try self.write(")\n            .real\n");
    ...
    fn generateCommentHandling(self: *LexerGenerator) !void {
        for (self.spec.rules.items) |rule| {
            if (!rule.is_simd) continue;

            // Extract the leading literal char from the pattern (e.g., ';' from "';' [^\n]*")
...
const ParserGenerator = struct {
    ...
    fn generateParserCode(self: *ParserGenerator, lexer_code: []const u8) ![]const u8 {
        ...
        // The lexer code starts with //! em Parser...
        ...
        try writer.writeAll(
            ...
            \\const maxArgs = bytecode.maxArgs;
            \\
        );
        ...
        try writer.writeAll(
            ...
            \\                        .id  = if (self.current.cat == .indent) self.current.pre else self.last_matched_id,
            \\                    } });
        ...
        for (self.symbols.items) |sym| {
            if (sym.kind == .terminal and sym.name.len > 0) {
                ...
                // Check if this is an @as directive type (like SET, KILL, FN, SYSVAR) vs a lexer token (like INDENT, INTEGER)
                // @as directives define rules like "cmd", "fn", "sysvar" - terminals matching these or their uppercase
                // versions should be skipped (they're handled by identToSymbol)
                var is_as_keyword = false;
                if (sym.name[0] >= 'A' and sym.name[0] <= 'Z') {
                    // Check if there's a corresponding lowercase nonterminal (indicates @as keyword)
                    for (self.symbols.items) |other| {
                        if (other.kind == .nonterminal and std.ascii.eqlIgnoreCase(sym.name, other.name)) {
                            is_as_keyword = true;
                            break;
                    ...
                    // Also check if it matches an @as directive rule name (uppercase)
                    if (!is_as_keyword) {
                        for (self.as_directives.items) |directive| {
        ...
        if (self.as_directives.items.len > 0) {
            ...
            // Non-cmd promotions always tried (fn/isv/ssvn appear after $ where
            // both ident and the keyword are valid — keyword must win for $SELECT etc.)
            for (self.as_directives.items) |directive| {
                if (std.mem.eql(u8, directive.token, "ident") and !std.mem.eql(u8, directive.rule, "cmd")) {
                    try writer.print("        if (self.try_ident_as_{s}(token, text)) |sym| return sym;\n", .{directive.rule});
                }
            }

            // Cmd promotion only when ident has no valid action (prevents command
            // keywords from hijacking variable names in argument positions)
            for (self.as_directives.items) |directive| {
                if (std.mem.eql(u8, directive.token, "ident") and std.mem.eql(u8, directive.rule, "cmd")) {
                    try writer.writeAll("        if (getAction(state, SYM_IDENT) != 0) return SYM_IDENT;\n");
                    try writer.print("        if (self.try_ident_as_{s}(token, text)) |sym| return sym;\n", .{directive.rule});
                }
            }
            ...
            for (self.as_directives.items) |directive| {
                ...
                const action_check = if (std.mem.eql(u8, directive.rule, "cmd")) "!= 0" else "> 0";
                try writer.print(
                    \\
                    ...
                    \\    }}
                    \\
                , .{ directive.rule, lang_name, directive.rule, directive.rule, action_check, directive.rule, action_check });
            }
        } else {
        ...
        for (self.as_directives.items) |directive| {
            ...
            for (self.symbols.items) |sym| {
                ...
                // Check if any rule matches this terminal (case-insensitive)
                for (self.symbols.items) |other| {
                    if (other.kind == .nonterminal and std.ascii.eqlIgnoreCase(sym.name, other.name)) {
            ...
            // Find fallback symbol (uppercase version of directive rule, e.g., "fn" → "FN")
            ...
            // Check if array will be mutated (has mappings or fallback)
            ...
            // Emit specific mappings for terminals with matching rules.
